...
ls "$_ROOT"/docs/mastery.md "$_ROOT"/docs/project-context.md 2>/dev/null
```
...
The Mastery framework is a single file that goes into `docs/`:
- `mastery.md` — the complete framework; the Agent Boot block at the top gives AI agents a 30-second orientation, the rest (templates included) loads on demand
...
> I need to set up the Mastery framework file. The framework is a single Markdown file that defines your project's development lifecycle.
>
...
echo "Downloaded mastery.md to docs/"
```
Verify the download isn't empty. If curl fails, tell the user to download manually from `https://github.com/RAiWorks/MASTERY.md` and place the file in `docs/`.

If B: Tell user to place `mastery.md` in the project's `docs/` folder. Wait for confirmation, then verify the file exists.
...
Read the **Agent Boot block** at the top of `docs/mastery.md` to understand the framework rules (context loading order).
...
```bash
cd "$_ROOT"
git add docs/mastery.md docs/project-discussion.md docs/project-context.md docs/project-roadmap.md docs/project-changelog.md AGENTS.md
git status
```
